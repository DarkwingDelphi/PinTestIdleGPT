...
const upgrades = [
  { name: "Flip", unlock: 10, cost: 10, effect: () => { passiveRate += 1; momentum += 1; }, unlocked: false },
  { name: "Drop Target", unlock: 20, cost: 20, effect: () => shotMult += 1, unlocked: false },
  { name: "Pop Bumper", unlock: 40, cost: 40, effect: () => momentum += 1, unlocked: false },
  { name: "Orbit Lane", unlock: 60, cost: 60, effect: () => shotMult += 2, unlocked: false },
  { name: "Vertical Upkick", unlock: 80, cost: 80, effect: () => bonusMult += 1, unlocked: false },
  { name: "Joel Node", unlock: 100, cost: 100, effect: () => dust += 1, unlocked: false },
  { name: "Jim Gate", unlock: 150, cost: 150, effect: () => shotMult += 3, unlocked: false },
  { name: "Kenny’s Circuit", unlock: 200, cost: 200, effect: () => bonusMult += 2, unlocked: false },
  { name: "Collapse Protocol (Locked)", unlock: 999, cost: 9999, locked: true, unlocked: false }
];
...
function updateUI() {
  ...
  upgrades.forEach(upg => {
    if (score >= upg.unlock || upg.unlocked) {
      upg.unlocked = true;
